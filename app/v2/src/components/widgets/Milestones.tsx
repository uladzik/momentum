...
function MilestoneCard({ m, onDelete }: { m: Milestone; onDelete: (id: number) => void }) {
  ...
  return (
    <div className="si rounded-2xl p-4 group bg-muted/50 border border-border">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={cn('text-xl font-mono font-bold tabular-nums', past ? 'text-muted-foreground/50' : 'text-foreground')}>
            {past ? '-' : ''}{days}<span className="text-xs font-normal ml-0.5 text-muted-foreground">d</span>
          </span>
          <button onClick={() => onDelete(m.id)} className="p-1 rounded opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive transition">
            <Trash2 size={11} aria-hidden="true" />
          </button>
        </div>
      </div>
      {!past && (
        <div className="h-[3px] rounded-full mt-3 overflow-hidden bg-muted">
          <div className="h-full rounded-full transition-all duration-1000" style={{ width: `${prog}%`, background: 'linear-gradient(90deg, var(--color-ring-month), var(--color-accent-midday))' }} />
        </div>
      )}
    </div>
  )
    ...
    <>
      <div className="flex flex-col gap-2">
        ...
        <button onClick={() => setShowForm(true)}
          className="w-full flex items-center justify-center gap-2 py-3 rounded-2xl border border-dashed border-border text-xs text-muted-foreground hover:border-muted-foreground/30 hover:text-muted-foreground transition bg-transparent cursor-pointer">
