...
export function Habits({ initialHabits }: Props) {
  ...
  return (
    <div className="flex flex-col gap-0.5">
      {cel && (
        <div className="si mb-3 px-3 py-2.5 rounded-xl text-center text-xs font-bold text-habit-streak"
          style={{ background: 'linear-gradient(90deg,transparent,oklch(var(--habit-streak) / 0.12),transparent)', backgroundSize: '200% 100%', animation: 'shimmer 1.5s ease-out' }}>
      ...
      <div className="flex items-center mb-2">
        ...
        <div className="flex gap-[3px] mr-6">
          {last7.map(d => (
            <div key={d.k} className="w-[22px] text-center text-[8px] font-mono font-medium uppercase text-muted-foreground/50">{d.l}</div>
          ))}
        </div>
      </div>
        ...
        return (
          <div key={hab.id} className="flex items-center py-1.5 group">
            <div className="flex-1 min-w-0 flex items-center gap-1.5">
              <span className={cn('text-[13px] truncate', done ? 'text-muted-foreground/50 line-through' : 'text-muted-foreground')}>
                {hab.name}
              </span>
              {st >= 2 && (
                <span className="flex items-center gap-0.5 text-[9px] font-bold shrink-0 text-habit-streak"
                  style={isMilestone ? { background: 'linear-gradient(90deg, var(--color-habit-streak), var(--color-accent-morning-2), var(--color-habit-streak))', backgroundSize: '200% 100%', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', animation: 'shimmer 2s ease-in-out infinite' } : {}}>
                  <Flame size={9} aria-hidden="true" />{st}
                </span>
              )}
              <span className={cn('text-[9px] font-mono ml-auto mr-2', wc >= 5 ? 'text-foreground' : 'text-muted-foreground/50')}>
                {wc}/7
              </span>
            </div>
              ...
              {last7.map(d => {
                ...
                return (
                  <button key={d.k} onClick={() => toggle(hab.id, d.k)}
                    className={cn('hcell size-[22px] rounded-[6px] flex items-center justify-center transition', isDone ? 'bg-primary text-primary-foreground' : isToday ? 'ring-1 ring-border' : 'bg-muted')}>
                    {isDone && <span className={cn(isPop && 'check-pop')}><Check size={9} /></span>}
                  </button>
                )
              })}
            </div>
            <button onClick={() => deleteHabit(hab.id)} className="ml-1.5 p-0.5 rounded opacity-0 group-hover:opacity-100 text-muted-foreground hover:text-destructive transition">
      ...
      {adding ? (
        ...
        <button onClick={() => setAdding(true)} className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition pt-2 bg-transparent border-none cursor-pointer">
